...
func sessionChannel(user *users.User, newChannel ssh.NewChannel) {
	...
	term.AddCommand("ls", commands.List(&controllableClients, term))
	term.AddCommand("help", commands.Help(term))
	term.AddCommand("exit", commands.Exit)
	term.AddCommand("connect", commands.Connect(user, defaultHandle, &controllableClients, term))
