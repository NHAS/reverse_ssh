...
func printHelp() {
	...
	fmt.Println("\t--trusted-proxy-cidr\tTrust X-Forwarded-For/X-Real-IP only from this CIDR; may be repeated or comma-separated")
...
func main() {
	...
	trustedProxyCIDRs, err := parseTrustedProxyCIDRs(options)
	if err != nil {
		fmt.Println(err)
		printHelp()
		return
	...
	server.Run(listenAddress, dataDir, connectBackAddress, autogeneratedConnectBack, tlscert, tlskey, insecure, enabledDownloads, tls, openproxy, timeout, trustedProxyCIDRs)
}

func parseTrustedProxyCIDRs(options terminal.ParsedLine) ([]*net.IPNet, error) {
	values, err := options.GetArgsString("trusted-proxy-cidr")
	...
	return parseTrustedProxyCIDRValues(values)
}

func parseTrustedProxyCIDRValues(values []string) ([]*net.IPNet, error) {
	var cidrs []*net.IPNet
	for _, value := range values {
		for _, part := range strings.Split(value, ",") {
			...
			cidr, err := parseTrustedProxyCIDR(part)
			if err != nil {
				return nil, err
			}
